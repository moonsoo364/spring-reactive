...
# 7장 리액티브 방식으로 데이터베이스 사용하기

## 데이터 조회를 위한 동기식 모델
리액티브 이전 시대의 애플리케이션에서 데이터 액세스를 구현하는 방법을 다시 한번 살펴보겠습니다. 또한 쿼리를 실행하고 처리할 때 클라이언트와 데이터베이스가 어떻게 통신하는 지, 이러한 통신의 어떤 부분을 비동기적으로 수행할 수 있는 지, 리액티브 프로그래밍 패턴을 적용하면 어떤 부분에서 이점을 얻을 수 있는 지 알아 보겠습니다. 데이터베이스 영속성은 몇 가지 추상화 계층으로 이루어져있으므로 이 모든 계층에 대해 알오본 후 리액티브로 하나씩 바꿔보는 시도를 해보겠습니다.

## 데이터베이스 액세스를 위한 와이어 프로토콜

임베디드 데이터베이스는 응용 프로그램 프로세스 내에 실행되므로 네트워크를 통한 통신이 필요하지 않습니다.

그러나 대부분 소프트웨어는 별도의 서버에서 프로세스로 실행되는 데이터베이스를 사용합니다. 응용프로그램은 데이터베이스 드라이버라고 하는 특수한 클라이언트 라이브러리를 사용해 외부 데이터베이스와 통신합니다. 또한 와이어 프로토콜은 데이터베이스 드라이버와 데이터베이스 자체가 통시하는 방법을 정의합니다.

와이어 프로토콜은 일반적으로 tcp/ip를 이용하도록 설계되었으므로 블로킹 방식으로 동작할 필요가 없습니다. 동기적인 HTTP 통신과 마찬가지로 프로토콜 자체가 차단되지는 않지만 클라이언트가 요청에 대한 결과를 반환하는 동안 차단하는 방식으로 동작하고 있을 뿐입니다. 또한 TCP는 슬라이딩 윈도우에 의해 구현된 흐름 제어를 통해 배압을 지원하는 비동기 프로토콜입니다.

배압 문제와는 별개로 클라이언트와 데이터베이스간에 큰 데이터를 전달하기 위해 완전히 다른 접근 방식을 사용할 수도 있습니다. 예를 들어 클라이언트가 수만 행의 데이터를 입력하거나 특정 데이터를 쿼리하기 위해 수백만 개의 행을 검색하는 경우를 예로 들 수 있습니다.  예제를 살펴보겠습니다.

- 데이터베이스 측에서 전체 결과 집합을 계산한 후 쿼리가 끝나자마자 데이터를 컨테이너에 넣고 컨테이너를 클라이언트에 전달하는 방식입니다. 이 접근법은 논리적으로 전혀 배압을 고려하지 않으며 데이터베이스 측에서 커다란 버퍼를 필요로 합니다. 또한 클라이언트 측에서는 전체 쿼리가 실행된 후 결과를 받을 수 있습니다.
- 클라이언트가 요청할 때 chunk로 결과 집합을 내보낼 수도 있습니다. 쿼리는 완전히 실행될 수 있으며 결과는 버퍼에 저장할 수 있습니다. 데이터베이스는 요청에 의해 청크가 채워지거나 클라이언트가 요구한 양의 데이터를 전달한 후에만 실행을 계속할 수 있습니다. 이 작동 방법은 메모리 버퍼 사용량을 줄일 수 있고 쿼리가 실행 중일 때 첫번 째 행을 반환할 수 있으며 논리적인 배압을 전파 가능하게 하고 쿼리를 취소할 수 있도록 합니다.

### 데이터베이스 드라이버

데이터베이스 와이어 프로토콜을 메서드 호출이나 콜백 리액티브 스트림과 같은 프로그래밍 언어로 구현한 라이브러리입니다.

동기식 블로킹으로 작성된 소프트웨어가 데이터 액세스에 대해서도 동일한 접근 방식을 사용한다는 것은 놀라운 일이 아닙니다. 이론적으로 데이터베이스 통신 프로토콜에 리액티브 설계를 적용하면 웹플럭스 WebClient와 유사한 이점을 얻을 수 있습니다.

### JDBC

JDBC를 사용하면 데이터베이스를 동일한 응용프로그램에서 사용할 수 있습니다.

SQL의 SELECT, CREATE, UPDATE, DELETE명령문을 초기화하고 실행할 수 있습니다. 명령문은 결과 행에 대한 이터레이터인 java.sql.ResultSet을 반환합니다. ResultSet 역방향 반복 및 무작위 액세스까지 고려해 설계됐짐나 호완성을 위해 드라이버가 모든행을 로드한 후에야 처리가 가능합니다.

jdbc는 select이외 쿼리에 대해 일괄처리를 허용합니다 하지만 jdbc는 동기적이고 블로킹되도록 설계됐으므로 커다란 데이터를 처리할 때는 이런 일괄처리가 도움이 되지 않습니다.

jdbc는 비즈니스 로직에서 사용할 수 있는 api로 설계됬지만 테이블과 행, 열을 직접 동작하기 때문에 도메인 기반 디자인이 권장하는 엔티티및 집계는 지원하지 않습니다.

### 커넥션 관리

최근 애플리케이션들은 jdbc에 직접 연결하지 않습니다. 대부분 커넥션 풀을 사용합니다. 배경은 매우 간단합니다. 새로운 커넥션을 맺는 비용이 많이 듭니다. 따라서 재사용을 허용하는 방식으로 관리되는 커넥션 캐시를 갖는 것이 좋습니다. 커넥션 비용은 크게 2가지로 구분됩니다.

1. 커넥션 초기화 프로세스에는 클라이언트 인증 및 권한 부여가 필요할 수 있으며 이는 귀중한 시간을 소모합니다.
2. 커넥션으로 인해 데이터베이스 비용이 발생할 수 있습니다. 에를 들어 PostgreSQL은 새로운 커넥션이 연결될 때 마다 새로운 프로세스를 생성합니다. 이 작업은 수백 밀리초가 걸리는 작업입니다.

자바 진영에서 일반적으로 쓰는 커넥션 풀은 아파치 커먼, dbcp2, 톰캣 jdbc, hikaricp입니다. hikaricp는 자바 진영에서 가장 빠른 커넥션풀로 인정받고 있습니다.

커넥션 풀은 jdbc 연결에 널리 사용되기는 하지만 데이터베이스 고유영역이 아닙니다. 예를 들어 오라클 데이터베이스 드라이버는 커넥션 멀티플렉싱을 허용하므로 단일 네트워크 연겨릉ㄹ 통해 여러 놀리적 커넥션을 연결할 수 있습니다.

### 리액티브 방식으로 관계현 데이터베이스 접근하기

jdbc는 자바 진영에서 데이터 액세스를 위한 기본 api이기 때문에 그 위에 구축된 모든 추상화 레이어들이 동작하는 기초가 됩니다. 리액티브 애플리케이션에서 사용할 데이터베이스 액세스 api가 필요합니다.

jdbc는 이런 솔루션이 없으며 이를 대신할 두 가지 api가 있습니다.

### 스프링 데이터 JDBC

스프링 데이터 jdbc는 @Query 어노테이션과 엔티티 라이프 사이클 이벤트를 통해 간단한 집계를 위한 CRUD를 제공합니다.

스프링 데이터  jdbc는 소규모 마이크로 서비스를 위한 간단한 영속성 계층을 구현하는 아주 작고 편리한 도구입니다. 이 모듈은담순함을 목표로  만들어졌습니다. 그래서 캐싱, 엔티티의 지연 로드 및 복잡한 엔티티와 같은 orm은 목표가 아닙니다. 이런 목적을 달성하기 위해 자바에는 JPA 라는 특별한 스펙이 있습니다.

### 스프링 데이터 jdbc 리액티브하게 만들기

스프링 데이터 jdbc는 완벽하게 블로킹 api인 jdbc를 필요로 하기 때문에 완전한 리액티브 스택에는 적합하지 않습니다.

## JPA

jpa는 영속성 계층을 정의하고 구성하는 표준입니다. JPA는 자체 API와 JPQL로 구성됩니다. JPQL은 데이터베이스 대신 리포지토리를 이용해 JPA 엔티티 객체를 쿼리하는 SQL과 같은 플랫폼 독립적 언어입니다.

데이터베이스 액세스의 표준이 JDBC라면 JPA는 코드의 객체를 데이터베이스 테이블에 매핑하는 ORM의 표준입니다. ORM은 내부적으로 JDBC 및 실행중에 생성중인 SQL을 사용하지만 이런 내부 구현은 응용프로그램 개발자가 몰라도 됩니다. JPA는 엔티티 뿐만 아니라 연관된 객체도 쉽게 로드할 수 있도록 엔티티 관계도 매핑할 수 있습니다.

가장 많이 사용되는 JPA 구현체는 Hibernate와 EclipseLink입니다.

Hibernate는 타임 스탬프와 natural ID에 대한 지원이 더 좋습니다 두 라이브러리는 멀티 테넌시를 지원합니다. 각 구현체가 제공하는 독점적인 기능을 사용하면 상호 교환이 불가능하다는 점은 염두에 두기 바랍니다.

순수 jdbc 대신 jpa를 구현하는 이유는 hibernate, eclipselink에서 제공해주는 캐싱 기능 때문입니다. 이 캐시를 이용하면 db 요청 횟수를 줄일 수 있습니다.

## 동기 모델의 한계

I/O요청이 HTTP이건 데이터베이스 요청이건 블로킹 방법으로 만드는 것은 낭비입니다. 또한 JDBC 기반 통시은 일반적으로 커넥션 풀을 사용해 쿼리를 병렬로 실행합니다. 데이터베이스가 멀티플렉싱을 할 수 이쓴ㄴ 비동기 논블로킹 드라이버를 제공하더라도 JDBC, JPA를 이용해서 최대한 성능을 얻을 순 없습니다.

요약하지만 기존의 잘 만들어진 JDBC, JPA 구현도 현대적인 리액티브 애플리케이션에서 병목이 될 수 있습니다. JDBC와 JPA는 런타임에서 지나치게 많은 스레드와 메모리를 사용하는 경향이 있습니다. 또한 긴 동기화 요청과 IO를 블로킹의 단점 해소를 위해 적극적인 캐싱이 필요합니다.

## 동기 모델의 단점

동기식 방식으로 영속성 계층을 구현하면 서버 자원을 효율적으로 사용하지는 못하지만 블로킹 웹 응용 프로그램을 구축할 때는 매우 유용합니다. 스프링 데이터 JPA는 데이터 영속성을 더 높은 수준의 도구를 제공하며 쿼리 번역및 트랜잭션 관리 등 엄청난 복잡성을 내부적으로 구현했습니다. 이 모든 것은 현대 응용프로그램 개발 방식을 상당히 단순하게 만들어줍니다.

## 스프링 데이터로 리액티브하게 데이터 접근하기

리액티브 리포지토리는 엔티티를 매개변수로 해서 저장 변경 및 삭제가 가능할 뿐만아니라 WebClient와 비슷한 방법으로 데이터 리포지토리를 사용하기를 원할 것입니다. 스프링 데이터 Commons 모듈에는 ReactiveCrudRepository 인터페이스가 포함돼 있습니다.

일반적인 블로킹 방식 대신 리액티브 데이터 액세스 계층을 사용할 때 어떤 이점이 있는 지 알아 보겠습니다.

- 효과적인 스레드 관리 : IO작업이 블로킹되지 않습니다.이는 보통 스레드가 줄어들고 스레드 스케줄링에 오버헤드를 적게 발생시키며 스레드 객체의 스택에 할당되는 메모리 공간이 줄어드는 결과를 가져옵니다. 이는 대량의 동시 연결을 처리할 수 있다는 것을 의미합니다.
- 첫번째 쿼리 결과에 대한 짧아진 대기 시간 : 쿼리가 끝나기 전에도 사용할 수 있습니다. 낮은 대기 시간 작업을 목표로 하는 검색 엔진 및 대화형 ui에 적합합니다.
- 낮은 메모리 사용량: 입출력 트래픽에 대한 쿼리를 처리할 때 버퍼링해야 할 데이터가 적기 때문에 유용합니다. 또한 클라이언트는 리액티브 스트림에서 수신을 거부할 수 있고 필요에 부합하는 데이터가 발생하는 즉시 네트워크를 통해 전송을 할 수 있으므로 송수신 데이터의 양을 줄일 수 있습니다.
- 배압 전파 : 클라이언트에게 새로운 데이터를 소비하는 데이터베이스의 능력을 알려줍니다.  또한 클라이언트가 데이터베이스 서버에게 쿼리 결과를 처리할 수 있는 클라이언트 능력을 알려줄 수도 있습니다. 이 경우 더 긴급한 작업이 대신 수행될 수 있습니다.
- 하나의 커넥션 공유 : 리액티브 클라이언트가 스레드를 전용하지 않으므로 다른 스레드에서 쿼리와 데이터 처리 작업을 보낼 수 있다는 이점이 있습니다. 다음으로 쿼리 및 커넥션 객체는 이러한 작업모드에 대해 내결함성이 있어야합니다. 어떤 스레드도 쿼리 개체에 대한 배타적 권한을 보유하지 않으며 클라이언트가 블로킹되지 않으므로 데이터베이스에 단일 와이어 연결을 공유할 수 있습니다. 그 결과로 커넥션 풀링은 필요 없을 수도 있습니다.
